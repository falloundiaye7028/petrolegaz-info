...
assert.equal(run().data.alertPreferencesEnabled,false);assert.equal(run().data.emailSendingEnabled,false);
env.PME_ALERT_PREFERENCES_ENABLED='true';assert.equal(run().data.alertPreferencesEnabled,true);
env.PME_ALERT_EMAIL_ENABLED='true';assert.equal(run().data.emailSendingEnabled,false);
delete env.PME_ALERT_PREFERENCES_ENABLED;
