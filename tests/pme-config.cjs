...
env.VERCEL_ENV='preview';assert.equal(run().status,200);env.VERCEL_ENV='production';assert.equal(run().status,503);
...
assert.equal(run().status,200);assert.equal(run().headers['Cache-Control'],'no-store');
